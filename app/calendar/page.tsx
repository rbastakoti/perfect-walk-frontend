...
import { DAY_START, DAY_END, todayEvents, detectGaps, getWalkSlots, formatTime } from "@/lib/calendar";
...
export default function CalendarPage() {
  ...
  const walkSlots = getWalkSlots(todayEvents);
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "var(--fg-muted)" }}>
            {todayEvents.length} events · {walkSlots.length} walk window{walkSlots.length !== 1 ? "s" : ""} detected
          </p>
        </div>
        {walkSlots.length > 0 && (
          ...
          {walkSlots.map((slot, i) => (
            <button key={i} type="button" onClick={() => router.push("/walk")}
              ...
              <div className="mt-2 flex items-center justify-between">
                <p className="text-sm text-white/70">{slot.end - slot.start} min free · tap for trails →</p>
