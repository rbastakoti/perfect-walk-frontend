...
interface Trail {
  ...
  dotY: number;
  lat?: number;
  lon?: number; // real coordinates for map tile
}
...
const FALLBACK_TRAILS: Trail[] = [
  {
    id: "riverside",
    name: "Riverside Loop Park",
    distance: "0.7 mi",
    duration: 18,
    difficulty: "Easy",
    tags: ["🚶 8 min walk", "🔄 Loop trail", "⛅ 72°F"],
    description: "A peaceful riverside loop for a quick mental reset.",
    color: "#6367FF",
    mapBg: "#0d2218",
    mapPath:
  ...
  },
  {
    ...
    duration: 32,
    difficulty: "Moderate",
    tags: ["🌲 Forest path", "↗ Gentle incline", "🐦 Wildlife"],
    description: "A shaded forest path, great for a mindful mid-day escape.",
    color: "#22c55e",
    mapBg: "#0d1f0e",
    mapPath:
  ...
  },
  {
    ...
    duration: 24,
    difficulty: "Easy",
    tags: ["🌊 Waterfront", "🌅 Scenic views", "🏃 Flat path"],
    description: "Walk along the harbour for an instantly calming experience.",
    color: "#0ea5e9",
    mapBg: "#080f1f",
    mapPath:
  ...
  {
    id: "hilltop",
    name: "Hilltop View Path",
    distance: "1.9 mi",
    duration: 45,
    difficulty: "Challenging",
    tags: ["⛰️ Elevation", "🌄 Summit views", "💪 Cardio boost"],
    description: "A rewarding climb with panoramic views at the top.",
    color: "#f59e0b",
    mapBg: "#1a1200",
    mapPath:
  ...
  },
  {
    ...
    duration: 14,
    difficulty: "Easy",
    tags: ["🌸 Open meadow", "☀️ Sunny", "🧘 Calm pace"],
    description:
...
function mapParkToTrail(park: any, idx: number): Trail {
  ...
  const difficulty: Trail["difficulty"] =
    duration < 20 ? "Easy" : duration < 35 ? "Moderate" : "Challenging";
...
function TrailMap({ trail }: { trail: Trail }) {
  ...
  if (trail.lat && trail.lon) {
    ...
    return (
      ...
      >
        ...
        <div
          className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-3 py-2 text-[10px] font-semibold"
          style={{
        ...
        >
          ...
          <span>
            {trail.distance} · {trail.duration} min loop
          </span>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      {/* name pill — top left */}
      <div
        className="absolute top-3 left-3 flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold backdrop-blur-md"
        style={{
      ...
      <div
        className="absolute top-3 right-12 rounded-full px-2.5 py-1 text-[10px] font-bold backdrop-blur-md"
        style={{
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to top, rgba(5,12,15,0.85), transparent)",
      ...
      >
        ...
        <div className="flex items-center gap-3 text-[11px]">
          <span className="font-semibold" style={{ color: trail.color }}>
            {trail.duration} min
          </span>
...
function TrailCard({
  ...
  return (
    <button
      ...
      style={{
        background: selected ? `${trail.color}14` : "var(--card)",
        border: `1.5px solid ${selected ? trail.color : "var(--border)"}`,
        boxShadow: selected ? `0 0 0 2px ${trail.color}22` : "var(--shadow)",
      }}
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <p
          className="text-sm font-bold leading-tight"
          style={{ color: selected ? trail.color : "var(--fg)" }}
        >
          {trail.name}
        </p>
        {selected && (
          <span
            ...
            style={{ background: `${trail.color}22`, color: trail.color }}
          >
            ✓
          </span>
        )}
    ...
    <div className="grid grid-cols-5 gap-2">
      {MOOD_EMOJIS.map((emoji, i) => (
        <button
          ...
          style={{
            background:
              value === i + 1 ? "var(--primary)" : "var(--primary-dim)",
            border: `1px solid ${value === i + 1 ? "transparent" : "var(--border)"}`,
          }}
        >
          <span className="text-2xl leading-none">{emoji}</span>
          <span
            className="text-[10px] font-semibold"
            style={{ color: value === i + 1 ? "#fff" : "var(--fg-muted)" }}
          >
            {MOOD_LABELS[i]}
          </span>
        </button>
    ...
    <div className="relative flex justify-center">
      <svg viewBox="0 0 240 240" className="w-52 md:w-64">
        ...
        <circle
          ...
          style={{
            transition: "stroke-dashoffset 0.8s linear, stroke 1s ease",
          }}
        />
        <text
          x="120"
...
export default function WalkPage() {
    ...
    fetch(`/api/parks?lat=${lat}&lon=${lon}&radius=5000`, {
      ...
      .then((data) => {
        ...
        const named = elements.filter(
          (p: any) => p.tags?.name || p.tags?.["name:en"],
        );
  ...
  useEffect(() => {
    ...
    if (cachedParks?.elements?.length) {
      const named = cachedParks.elements.filter(
        (p) => p.tags?.name || p.tags?.["name:en"],
      );
  ...
  const captureWalkingSession = (afterMoodValue: number | null) => {
    ...
    const sessionData = {
      ...
      moodImprovement:
        afterMoodValue && beforeMood ? afterMoodValue - beforeMood : 0,
  ...
  const saveWalkSessionLocally = (sessionData: any) => {
    if (typeof window === "undefined") return;
    try {
      const key = "pw-walk-sessions-local";
      const existingRaw = localStorage.getItem(key);
      const existing = existingRaw ? JSON.parse(existingRaw) : [];
      const next = Array.isArray(existing)
        ? [sessionData, ...existing].slice(0, 200)
        : [sessionData];
      localStorage.setItem(key, JSON.stringify(next));
    } catch (error) {
      console.error("Failed to persist local walk session:", error);
    }
  };
  ...
  useEffect(() => {
    if (!running) return;
    const id = window.setInterval(
      () => setBreathIdx((prev) => (prev + 1) % breathingMessages.length),
      8000,
    );
    return () => window.clearInterval(id);
  }, [running]);
      ...
      <div className="animate-fade-in-up space-y-5">
              ...
              {!loadingParks && locationStatus === "error" && (
                <button
                  ...
                  onClick={() =>
                    navigator.geolocation?.getCurrentPosition(
                      (p) =>
                        fetchNearbyParks(p.coords.latitude, p.coords.longitude),
                      () => setLocationStatus("denied"),
        ...
        <div className="pw-card space-y-4">
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, var(--primary-dim), rgba(255,219,253,0.08))",
          ...
          <button
            ...
            onClick={() => {
              setRunning(true);
              setBreathIdx(0);
              setRemaining(selectedTrail.duration * 60);
              setPhase("timer");
            }}
  ...
  if (phase === "timer") {
    return (
      <div className="animate-fade-in-up space-y-5">
        ...
        {running && (
          <div
            className="rounded-2xl px-5 py-4 text-center animate-fade-in"
            style={{
              background: "var(--primary-dim)",
              border: "1px solid var(--border)",
            }}
    ...
    <div className="animate-fade-in-up space-y-5">
      <div className="pw-hero p-6 text-center">
        ...
        <p className="mt-1 text-sm" style={{ color: "var(--fg-muted)" }}>
          {selectedTrail.name} · {Math.round((totalSeconds - remaining) / 60)}{" "}
          min · {selectedTrail.distance}
        </p>
      </div>
      ...
      <div className="pw-card space-y-3">
        ...
        {afterMood && beforeMood && afterMood > beforeMood && (
          <div
            className="rounded-xl px-4 py-3 text-center animate-fade-in"
            style={{ background: "rgba(34,197,94,0.1)", color: "#16a34a" }}
          >
            <p className="text-sm font-bold">
              +{afterMood - beforeMood} mood lift 🌱
            </p>
            <p className="text-xs mt-0.5">
              The walk worked. Your data is being tracked.
            </p>
          </div>
        )}
      </div>
        ...
        <button
          ...
          onClick={async () => {
            ...
            if (!session?.user) {
              ...
              saveWalkSessionLocally(walkData);
              setSaved(true);
            ...
            try {
              ...
              // Backend unavailable: keep experience working with local fallback.
              saveWalkSessionLocally(walkData);
              setTimeout(() => router.push("/dashboard"), 1200);
            }
          }}
